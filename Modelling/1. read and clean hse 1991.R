...
df <- read.table("/Users/elenamariani/Documents/data/hse_1991_2009/tab/1991_2009hse.tab", sep = "\t", header = TRUE)

# filter to 1991-92, adults with valid weight and height

df_1991 <- df %>% filter(year == "1992")
nrow(df_1991)

df_valid <- df_1991 %>% filter(!is.na(wtvalid) & !is.na(htvalid))
nrow(df_valid)

# save clean version

write_csv(df_valid, "/Users/elenamariani/Documents/working/weight_loss_modelling/processed_data/hse_1991_clean.csv")


# Explore BMI distribution 

bmi90 <- df_valid[,"bmivalid"] %>% na.omit() %>% as.numeric()
...
newDist <- rinvgamma(nrow(df_valid), shape = 36.15893, scale = 897.80151)
