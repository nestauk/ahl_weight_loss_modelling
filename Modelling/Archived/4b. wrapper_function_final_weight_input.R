...
dat_sim <- dat %>% 
  mutate(age_plus = age_est + 10,
         baseIntake = 1.63*case_when((sex_label == "male" & age_est >= 18 & age_est < 30) ~ 113 + 14.4*wtval + 313*htval/100,
                                ...
                                TRUE ~ 0), 
         newIntake = 1.63*case_when((sex_label == "male" & age_plus >= 18 & age_plus < 30) ~ 113 + 14.4*target + 313*htval/100,
                                ...
                                TRUE ~ 0)) %>% mutate(calRed = (newIntake - baseIntake)/baseIntake)

dat_sim %>% group_by(bmi_class) %>% summarise(mean = mean(calRed))

funScenario <- function(id, w0, height, sex, age, w1, days){
  
  bmi <- w0/height/height*10000
  
  RMR0 <- case_when((sex == "M" & age >= 18 & age < 30) ~ 113 + 14.4*w0 + 313*height/100,
                   (sex == "M" & age >= 30 & age < 60) ~ -137 + 11.4*w0 + 541*height/100,
                   (sex == "M" & age >= 60) ~ -256 + 11.4*w0 + 541*height/100,
                   (sex == "F" & age >= 18 & age < 30) ~ 282 + 10.4*w0 + 615*height/100,
                   (sex == "F" & age >= 30 & age < 60)~ -11.6 + 8.18*w0 + 502*height/100,
                   (sex == "F" & age >= 60) ~ 10.7 + 8.52*w0 + 421*height/100,
                   TRUE ~ 0)
 ...
 tout <- (seq(0, days, by = days/50)) # calculate 1 month intervals (30 days approximately)
  ...
  CalRedShare <- seq(0.01,0.50,by=0.01)  # List of potential calorie reduction values
...
dat_obese <- dat %>% filter(bmi_class == "obese") %>% head(1)
