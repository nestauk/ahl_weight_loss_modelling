...
funScenario <- function(id, w0, height, sex, age, CalRed, days){
...
# flat 90 kcal reduction

start_time <- Sys.time()
reduction_90 <- do.call("rbind", mapply(funScenario, 
                                        id = df$X, 
                                        w0 =df$Wt_est, 
                                        height = df$Ht_est, 
                                        sex = df$Sex_letter, 
...
with(reduction_90, table(BMI_class, final_BMI_class)) %>% prop.table(1)*100


reduction_90 %>% 
  select(BMI_est, final_BMI, Sex) %>% 
  melt(., id.vars = c("Sex")) %>% 
  mutate(BMI_type = ifelse(variable == "BMI_est", "Now", "In 3 Years")) %>% 
  ggplot(., aes(x = value, group = BMI_type, color = BMI_type)) +
...
write_csv(reduction_90, "C:\\Users\\Elena.Mariani\\Documents\\Projects\\ahl_weight_loss_modelling\\Output\\hse_modelling_3years_90kcal.csv")
